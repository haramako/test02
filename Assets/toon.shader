    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(v2f i) : COLOR {
                ...
                diff = tex2D(_GradientMap, diff.xy);

                float nh = saturate(dot(normal, h));
                float spec = 0;//pow(nh, _Shininess * 128.0) * gloss;

                fixed3 lt = _LightColor0.rgb * diff + UNITY_LIGHTMODEL_AMBIENT.rgb;
                lt = min(lt,fixed3(1));
